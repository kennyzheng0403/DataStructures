...
public class ArrayDeque<T> {
    ...
    public void addFirst(T data) {
        ...
        if (size == 0) {
            ...
            for (int i = 0; i < backingArray.length; i++) {
                ...
                j = j + 1 % backingArray.length;
            }
            backingArray = newArr;
            backingArray[front - 1 % backingArray.length] = data;
            front = front - 1 % backingArray.length;
            size++;

        } else {
            int temp = mod(front - 1, backingArray.length);
            backingArray[temp] = data;
            front = front - 1 % backingArray.length;
            size++;
    ...
    public void addLast(T data) {
        ...
        if (size == 0) {
            ...
            int temp = mod(front + size, backingArray.length);
            backingArray[temp] = data;
            size++;
